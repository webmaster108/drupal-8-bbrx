<?php

namespace Drupal\clearstone\Form;

use Drupal\Core\Form\FormBase;
use Drupal\Core\Form\FormStateInterface;
use Drupal\Core\Url;
use Drupal\Core\Link;

/**
 * Class ClearstoneFormMergesForm.
 */
class ClearstoneFormMergesForm extends FormBase {

  /**
   * {@inheritdoc}
   */
  public function getFormId() {
    return 'clearstone_form_merges_form';
  }

  /**
   * {@inheritdoc}
   */
  public function buildForm(array $form, FormStateInterface $form_state) {

    $form['merges'] = [
      '#type' => 'fieldset',
      '#title' => t('Manage Merges'),
      '#collapsible' => FALSE,
      '#description' => t('Create and manage merges that forms will be exported into.'),
    ];
    $form['merges']['merge_title'] = [
      '#type' => 'textfield',
      '#title' => t('Title of Merge'),
      '#required' => TRUE,
    ];
    $form['merges']['filename'] = [
      '#type' => 'textfield',
      '#title' => t('Filename'),
      '#required' => TRUE,
      '#description' => t('Leave off date/time and extension. ex. PFire2CLR_enrollazmapd15'),
    ];
    $form['merges']['output_folder'] = [
      '#type' => 'textfield',
      '#title' => t('Output Folder'),
      '#description' => t('ex: Full folder stucture, /var/www/.'),
      '#required' => TRUE,
    ];
    $form['merges']['root_tag'] = [
      '#type' => 'textfield',
      '#title' => t('Root Tag'),
      '#required' => TRUE,
      '#description' => t('Leave off <>. ex. hp_online_enrollement'),
    ];
    $form['merges']['record_tag'] = [
      '#type' => 'textfield',
      '#title' => t('Record Tag'),
      '#required' => TRUE,
      '#description' => t('Leave off <>. ex. enrollee'),
    ];
    $form['submit'] = [
      '#type' => 'submit',
      '#value' => t('Save'),
    ];

    $db_select = \Drupal::database()->select('clearstone_form_merge', 'm')
      ->fields('m')
      ->orderBy('id', 'DESC');
    $ids = $db_select->execute()->fetchAll();
    $output = [];
    if (!empty($ids)) {
      foreach ($ids as $key => $data) {
        $edit_url = Url::fromRoute('clearstone.form_merges_edit', ['mid' => $data->id]);
        $edit = Link::fromTextAndUrl(t('Edit'), $edit_url);

        $delete_url = Url::fromRoute('clearstone.form_merges_delete', ['mid' => $data->id]);
        $delete = Link::fromTextAndUrl(t('Delete'), $delete_url);
        $output[$key] = [
          'id' => [$data->id],
          'title' => [$data->title],
          'filename' => [$data->filename],
          'root_tag' => [$data->root_tag],
          'record_tag' => [$data->record_tag],
          'output_folder' => [$data->output_folder],
          'edit' => [$edit],
          'delete' => [$delete],
        ];

      }
    }

    $header = [
      'id' => t('ID'),
      'title' => t('Title'),
      'root_tag' => t('Root Tag'),
      'record_tag' => t('Record Tag'),
      'filename' => t('Filename'),
      'output_folder' => t('Output Folder'),
      'edit' => t('Edit'),
      'delete' => t('Delete'),
    ];
    $form['table'] = [
      '#type' => 'tableselect',
      '#header' => $header,
      '#options' => $output,
      '#empty' => t('No submissions found'),
    ];
    return $form;
  }

  /**
   * {@inheritdoc}
   */
  public function validateForm(array &$form, FormStateInterface $form_state) {

  }

  /**
   * {@inheritdoc}
   */
  public function submitForm(array &$form, FormStateInterface $form_state) {
    $values = $form_state->getValues();
    if (!empty($values)) {
      $title = $values['merge_title'];
      $filename = $values['filename'];
      $root_tag = $values['root_tag'];
      $record_tag = $values['record_tag'];
      $output_folder = $values['output_folder'];
      $array = [
        'title' => $title,
        'filename' => $filename,
        'output_folder' => $output_folder,
        'record_tag' => $record_tag,
        'root_tag' => $root_tag,
      ];

      $update_data = \Drupal::database()->insert('clearstone_form_merge')
        ->fields($array)
        ->execute();
      \Drupal::messenger()->addMessage('Form Export data saved successsfully.');
    }
    else {
      \Drupal::messenger()->addMessage(t('Some error occurred'), 'error');
    }
  }

}
